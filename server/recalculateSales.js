...
const recalculateSales = async () => {
  try {
    ...
    console.log(
      DRY_RUN
        ? "👁️  MODO PREVIEW - NO SE GUARDARÁ NADA"
        : "🚀 MODO ACTUALIZACIÓN - SE GUARDARÁ EN LA BD",
    );
    console.log("=".repeat(100));
    console.log();

    const sales = await Sale.find({ paymentStatus: "confirmado" }).sort({
      createdAt: 1,
    });
    ...
    for (const sale of sales) {
      ...
      if (!sale.distributor) {
        ...
        newDistributorProfit =
          (sale.salePrice - newDistributorPrice) * sale.quantity;

        // Ganancia del admin = Precio Venta - Precio Compra - Ganancia Distribuidor
        newAdminProfit =
          (sale.salePrice -
            sale.purchasePrice -
            (sale.salePrice - newDistributorPrice)) *
          sale.quantity;
      }
      ...
      const priceDiff = Math.abs(
        (oldDistributorPrice || 0) - (newDistributorPrice || 0),
      );
      ...
      if (adminDiff > 0.01 || distDiff > 0.01 || priceDiff > 0.01) {
        changes.push({
          sale,
          old: {
            admin: oldAdminProfit,
            dist: oldDistributorProfit,
            price: oldDistributorPrice,
          },
          new: {
            admin: newAdminProfit,
            dist: newDistributorProfit,
            price: newDistributorPrice,
          },
          diff: {
    ...
    if (changes.length === 0) {
      console.log(
        "✅ ¡Todas las ventas están correctas! No hay nada que actualizar.\n",
      );
    ...
    console.log(`${"-".repeat(100)}`);
    console.log(
      `🔍 VENTAS QUE NECESITAN RECALCULARSE: ${changes.length} de ${sales.length}`,
    );
    console.log(`${"-".repeat(100)}\n`);
    ...
    changes.forEach((item, i) => {
      ...
      console.log(
        `   Fecha: ${sale.saleDate?.toISOString().split("T")[0] || "N/A"}`,
      );
      console.log(
        `   Precio Venta: $${sale.salePrice.toLocaleString("es-CO")} x ${sale.quantity}`,
      );
...
`);
    ...
    console.log(`${"=".repeat(100)}`);
    console.log(
      `Admin Profit:       ${totalAdminDiff >= 0 ? "+" : ""}$${totalAdminDiff.toLocaleString("es-CO")}`,
    );
    console.log(
      `Distributor Profit: ${totalDistDiff >= 0 ? "+" : ""}$${totalDistDiff.toLocaleString("es-CO")}`,
    );
    console.log(`${"=".repeat(100)}\n`);
