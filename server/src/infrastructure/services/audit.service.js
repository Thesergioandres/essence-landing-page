...
class AuditService {
  ...
  /**
   * Log de operaciones con employees
   */
  static async logEmployee(user, action, employee, req = null) {
    const descriptions = {
      employee_created: `Employee "${employee.name}" creado`,
      employee_updated: `Employee "${employee.name}" actualizado`,
      employee_deleted: `Employee "${employee.name}" eliminado`,
      employee_activated: `Employee "${employee.name}" activado`,
      employee_deactivated: `Employee "${employee.name}" desactivado`,
    };
  ...
  static async logStock(user, action, stock, metadata = {}, req = null) {
    ...
    const employeeName =
      stock.employee?.name || metadata.employeeName || "Employee";
