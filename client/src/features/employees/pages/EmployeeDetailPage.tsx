...
const EmployeeDetail = () => {
  ...
  useEffect(() => {
    const loadStock = async () => {
      try {
        if (!id) return;
        const response = await stockService.getEmployeeStock(id);
        setStock(response);
      } catch (err: any) {
    ...
    const loadSales = async () => {
      try {
        if (!id) return;
        const response = await saleService.getEmployeeSales(id);
        ...
        const totalProfit = response.sales.reduce(
          (sum: number, sale: Sale) => sum + sale.employeeProfit,
          0
        );
    ...
    const loadBranchesData = async () => {
      try {
        ...
        const employeeMembership = membersResponse.members.find(
          (m: { user: any }) =>
            (typeof m.user === "object" && m.user?._id === id) || m.user === id
        );

        if (employeeMembership) {
          setMembership(employeeMembership as any);
          // Inicializar bodegas seleccionadas
          const branchIds = (
            (employeeMembership as any).allowedBranches ||
            employeeMembership.branches ||
            []
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-400">{employee.email}</p>
        </div>
        ...
        {activeTab === "info" && (
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-lg text-white">{employee.name}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400">
                Email
              </label>
              <p className="mt-1 text-lg text-white">{employee.email}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400">
                Teléfono
            ...
            <div>
              ...
              <p className="mt-1 text-lg text-white">
                {employee.address || "No especificada"}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400">
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {stock.map(item => (
                      <tr key={item._id} className="hover:bg-gray-900/30">
                        ...
                        <td className="whitespace-nowrap px-6 py-4 text-gray-200">
                          {typeof item.product === "object"
                            ? formatCurrency(item.product.employeePrice || 0)
                            : "N/A"}
                        </td>
                        <td className="whitespace-nowrap px-6 py-4 text-gray-200">
                          {item.lowStockAlert}
