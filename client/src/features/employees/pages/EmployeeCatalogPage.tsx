...
export default function EmployeeCatalog() {
      ...
      try {
        ...
        const normalizedProducts = (productsList || []).map(
          (product: ProductWithStock) => ({
            ...product,
            totalStock: product.employeeStock ?? product.totalStock ?? 0,
          })
        );
  ...
  useEffect(() => {
    ...
    if (user?._id) {
      setPublicCatalogUrl(
        `${window.location.origin}/staff-catalog/${user._id}`
      );
    }
  }, []);
  ...
  const handleExportCatalog = async () => {
    ...
    try {
      ...
      console.error("Error exporting employee catalog PDF", error);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (inStockOnly) {
      list = list.filter(p => (p.employeeStock ?? 0) > 0);
    }
          ...
          <div className="bg-linear-to-br rounded-2xl border border-white/10 from-white/10 via-white/5 to-white/0 p-5 shadow-2xl backdrop-blur-xl">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => navigate("/staff/share-catalog")}
                  className="rounded-lg border border-white/10 bg-gray-950/60 px-4 py-2 text-sm font-semibold text-gray-100 transition hover:bg-white/10"
                >
                  Ver opciones
                </button>
