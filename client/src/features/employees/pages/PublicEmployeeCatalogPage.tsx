...
export default function PublicEmployeeCatalog() {
  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        setLoading(true);
        const response = await employeeService.getPublicCatalog(
          employeeId || ""
        );
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Catalogo de ${employee?.name || "productos"}`,
  ...
  const handleExportCatalog = async () => {
    ...
    try {
      ...
      console.error(
        "Error exporting public employee catalog PDF",
        exportError
      );
          ...
          <div className="flex flex-col gap-4 text-center">
            <div>
              ...
              <h1 className="text-4xl font-bold text-white">
                Catalogo de {employee?.name || "productos"}
              </h1>
              {employee?.phone && (
              ...
              {employee?.email && (
                <p className="mt-1 text-gray-400">Email: {employee.email}</p>
              )}
            </div>
